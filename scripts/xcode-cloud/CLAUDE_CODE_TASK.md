...
```json
{
  ...
  "environmentVariables": [
    ...
    { "key": "ELEVENLABS_API_KEY", "isSecret": true }
  ]
}
```
...
if [ -z "$ELEVENLABS_API_KEY" ]; then
  echo "⚠️  ELEVENLABS_API_KEY not set — using placeholder"
  ELEVENLABS_API_KEY="MISSING_IN_CI"
fi
...
// ElevenLabs API key — used for high-quality neural TTS
// Get yours at: https://elevenlabs.io/app/api/api-keys
ELEVENLABS_API_KEY = REPLACE_ME_ELEVENLABS_KEY
EOF
...
So that the `Secrets.xcconfig` is actually loaded by Xcode at build time. Without this step, `OPENAI_API_KEY`, `ANTHROPIC_API_KEY`, and `ELEVENLABS_API_KEY` won't be injected into the app binary.
...
# Edit Secrets.xcconfig with real OpenAI + Anthropic + ElevenLabs keys
...
5. Repeat for:
   ...
   - Value: paste the real ElevenLabs key (from https://elevenlabs.io/app/api/api-keys)
   - ☑️ **Secret**
   - Save
...
✓ Xcode Cloud setup complete
  ...
  • Env vars OPENAI_API_KEY, ANTHROPIC_API_KEY, ELEVENLABS_API_KEY configured (Secret)
